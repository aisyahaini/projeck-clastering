...
def preprocessing_data():
    ...
    st.success("Dataset telah disimpan ke dalam file 'dataset-fix.csv'")
...
def pengujian_data():
    ...
    # Plotting the results including the optimal value
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.plot(range(2, max_k), db_scores, marker='o', color='tab:green', label='Davies-Bouldin Index')
    ax.axvline(x=optimal_k_davies, color='red', linestyle='--', label='Optimal k')
    ax.set_xlabel('Number of Clusters (k)')
    ax.set_ylabel('Davies-Bouldin Index')
    ax.legend()
    ax.set_title('Davies-Bouldin Index for Different Numbers of Clusters')

    # Display the plot
    st.pyplot(fig)

    # Display the Davies-Bouldin Index value
    
    
    # Create a dictionary with the values
    index_values = {
        "Metode": ["Silhouette Index", "Davies-Bouldin Index"],
        "Nilai Pengujian": [silhouette_scores[optimal_k_silhouette - 2], db_scores[optimal_k_davies - 2]]
    }
